...
class DepthRefinementModel(nn.Module):
    def __init__(self, args, bundle_dataset):
        ...
        self.gaussian_weights = torch.tensor(gkern(args.patch_size, args.patch_size / 5).flatten(), device=args.device)[
            None, :, None
        ]  # [1,N,1]
    ...
    def get_visualization(self, y_samples=128, x_samples=96, coords=None):
        ...
        if coords is None:
            y, x = torch.meshgrid(
                torch.linspace(0, args.height - 1, y_samples), torch.linspace(0, args.width - 1, x_samples)
            )
        ...
        ref_rays, qry_rays, mlp_rays, qry_conf = self.forward(
            self.ref_img,
            self.ref_depth,
            torch.ones_like(self.ref_depth),
            self.ref_intrinsics,
            [0],
            coords=coords,
            ref=True,
    ...
    def forward(self, qry_img, qry_depth, qry_conf, qry_intrinsics, qry_idx, coords=None, ref=False):
        ...
        ref_from_qry_rays = resample_rgb(
            ref_from_qry_rays, self.ref_img, patch_size=self.args.patch_size
...
class ImplicitDepthNet(nn.Module):
    ...
    def forward(self, rays, confidence):
        ...
        N = (M - 1) // 2  # 60
        mlp_in = torch.cat(
            (yx_encoded, rays[:, [4 + N, 4 + N + M, 4 + N + 2 * M]]), dim=1
        )  # Select center rgb values (1, 42, 4096)
